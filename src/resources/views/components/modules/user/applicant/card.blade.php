<div class="applicants-list-item">
    <div class="applicants-list-item-info">
        <div class="applicant-fio">
        ...
        <div class="applicant-birthday">
            <h3>Дата рождения: </h3>
            <p>- {{ getFormatedDate($applicant->birthday) ??  'Информация отсутствует' }}</p>
        </div>
        <div class="applicant-salary">
            <h3>Желаемая зарплата: </h3>
            <p>- {!! getMoney($applicant->salary) ?? 'Информация отсутствует' !!}</p>
        </div>
        <div class="applicant-city">
            <h3>Город проживания:</h3>
            <p>- {{ $applicant->city->name ?? 'Информация отсутствует' }}</p>
        </div>
        ...
        <div class="applicant-education">
            <h3>Образование:</h3>
            @if($applicant->universities->isNotEmpty() || $applicant->collages->isNotEmpty())
                @if($universities = $applicant->universities)
                    @foreach($universities as $university)
                        <p>- {{ $university->name }}</p>
                    @endforeach
                @endif
                @if($collages = $applicant->collages)
