    ...
    <div class="controls">
        @auth
            ...
            <div>
                ...
                <a href="{{ $user->hasRole('Соискатель') ? route('user.profile') : route('platform.main')}}" @class(['active-page' => request()->routeIs('user.profile') || request()->routeIs('user.edit') || request()->routeIs('user.password.edit')])>{{ $user->fullName(true) }}</a>
            </div>
        @else
            <div>
                <svg width="64px" height="64px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
