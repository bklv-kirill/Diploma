...
class LoadUserAvatar extends Command
{
    ...
    public function handle(LoadUserAvatarIfExistsAction $loadUserAvatarIfExists)
    {
        ...
        $users->each(function (User $user) use (
            $avatarsPaths,
            $loadUserAvatarIfExists
        ) {
            $avatarPath = Arr::random($avatarsPaths);
            $avatarName = File::name(Arr::random($avatarsPaths)).'.'
                .File::extension(Arr::random($avatarsPaths));

            $uploadAvatar = new UploadedFile(Storage::disk('images')
